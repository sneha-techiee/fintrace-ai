...
#         ↓
# subtract/add the incident effect
#         ↓
...
from data_gen.models import LedgerEntry, DashboardMetric
from data_gen.detect_incidents import detect_incidents

def simulate_missing_refund(dashboard_metrics, ledger_entries, target_merchant_id):
    ...
    for dashboard_metric in dashboard_metrics:
        ...
        if dashboard_metric.merchant_id == target_merchant_id:
            ...
            and dashboard_metric.period_start <= entry.timestamp <= dashboard_metric.period_end
):
...
# It searches the ledger for the first refund belonging to our target merchant and stores its amount in:
# refund_amount
# Then break stops searching once we've found one.
        else:
            faulty_dashboard_metrics.append(dashboard_metric)
    return faulty_dashboard_metrics
if __name__ == "__main__":
   ...
   for incident in incidents:
    print(
        ...
        f"Expected: {incident.currency} {incident.expected_revenue} | "
        f"Actual: {incident.currency} {incident.actual_revenue} | "
        f"Difference: {incident.currency} {incident.discrepancy} | "
